...
def clean_urls(urls, extensions, placeholder):
    ...
    for url in urls: 
        ...
        if not has_extension(cleaned_url, extensions):
            ...
            query_params = parse_qs(parsed_url.query, keep_blank_values=True)  # Boş değerleri koru
            if query_params:
                cleaned_params = {key: placeholder if not value else placeholder for key, value in query_params.items()}
